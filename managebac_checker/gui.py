...
class ConfigDialog(tk.Toplevel):
    ...
    def _create_basic_tab(self):
        ...
        self._create_entry(scrollable_frame, "📋 Formats | 格式:", self.format_var, 
                         help_text="Comma-separated: html,json,markdown,console")
        
        self.output_dir_var = tk.StringVar(value="reports")
        output_frame = self._create_entry(scrollable_frame, "📁 Output Directory | 输出目录:", self.output_dir_var)
    ...
    def _create_advanced_tab(self):
        ...
        self._create_entry(self.email_settings_frame, "📮 SMTP Server | SMTP服务器:", self.smtp_server_var)
        
        self.smtp_port_var = tk.IntVar(value=587)
        self._create_entry(self.email_settings_frame, "🔌 SMTP Port | SMTP端口:", self.smtp_port_var)
        
        self.smtp_username_var = tk.StringVar()
        self._create_entry(self.email_settings_frame, "👤 SMTP Username | SMTP用户名:", self.smtp_username_var)
        
        self.smtp_password_var = tk.StringVar()
        self._create_entry(self.email_settings_frame, "🔐 SMTP Password | SMTP密码:", self.smtp_password_var, show="*")
        
        self.recipients_var = tk.StringVar()
        self._create_entry(self.email_settings_frame, "📬 Recipients | 收件人:", self.recipients_var,
    ...
    def _load_config(self):
        ...
        try:
            if os.path.exists('.env'):
                ...
                # Email settings
                self.email_enabled_var.set(os.getenv('ENABLE_EMAIL_NOTIFICATIONS', 'false').lower() == 'true')
...
class ManageBacGUI:
    ...
    def _create_menu_bar(self):
        ...
        file_menu.add_command(label="📊 Open Reports Folder | 打开报告文件夹", command=self._open_reports_folder)
        file_menu.add_separator()
        ...
        tools_menu.add_command(label="🔄 Refresh Assignments | 刷新作业", command=self._refresh_assignments)
        ...
        view_menu.add_command(label="🌞 Light Theme | 浅色主题", command=lambda: self._change_theme('light'))
        view_menu.add_command(label="🌙 Dark Theme | 深色主题", command=lambda: self._change_theme('dark'))
    ...
    def _create_main_layout(self):
        # Main container
        main_container = tk.PanedWindow(self.root, orient='horizontal', bg=self.theme.get_color('bg'))
    ...
    def _on_search_change(self, *args):
        ...
        filtered = [
            ...
            if search_term in a.get('title', '').lower() or
               search_term in a.get('course', '').lower()
        ]
        self._display_assignments(filtered)
    ...
    def _check_assignments(self):
        ...
        if not self.config:
            messagebox.showwarning(
                "Warning | 警告",
                "⚠️ Please configure settings first!\n⚠️ 请先配置设置！"
            )
            self._open_settings()
            return
    ...
    def _on_assignments_loaded(self):
        ...
        self._update_status(f"✅ Found {len(self.assignments)} assignments | 找到{len(self.assignments)}个作业")
    ...
    def _run_ai_analysis(self):
        ...
        def ai_thread():
            try:
                ...
                for assignment in self.assignments:
                    assignment['ai_suggestion'] = f"Focus on completing this {assignment.get('priority', 'medium')} priority assignment. Consider breaking it into smaller tasks."
