...
class EnhancedManageBacGUI(ManageBacGUI):
    ...
    def _show_preferences(self):
        ...
        AnimatedButton(
            button_frame, self.theme, text="💾 Save | 保存", command=save_preferences
    ...
    def _toggle_system_tray(self):
        ...
        if self.tray_manager and self.tray_manager.running:
            ...
            messagebox.showinfo(
                "System Tray | 系统托盘", "❌ System tray disabled\n❌ 系统托盘已禁用"
            )
        else:
            self._setup_system_tray()
            messagebox.showinfo(
                "System Tray | 系统托盘", "✅ System tray enabled\n✅ 系统托盘已启用"
            )
    ...
    def _toggle_auto_check(self):
        ...
        messagebox.showinfo(
            "Auto Check | 自动检查", f"🔄 Auto check {status}\n🔄 自动检查{status_zh}"
        )
    ...
    def run(self):
        ...
        try:
            ...
            if self.tray_manager:
                self.notification_manager.send_notification(
                    "ManageBac检查器", "应用程序已启动 - 点击系统托盘图标访问功能"
                )
