...
from .improved_system_tray import ImprovedSystemTrayManager, ImprovedNotificationManager
...
class ProfessionalCard(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        status_label = tk.Label(
            ...
            font=('SF Pro Text', 9, 'bold') if sys.platform == 'darwin' else ('Segoe UI', 9, 'bold'),
            bg=status_colors.get(status, self.theme.get_color('secondary')),
            fg='white',
            padx=12,
            pady=4
        ...
        priority_emoji, priority_color = priority_colors.get(priority, ('⚪', self.theme.get_color('secondary')))
...
class ProfessionalManageBacGUI:
    ...
    def _create_professional_menu(self):
        ...
        menubar = tk.Menu(self.root, bg=self.theme.get_color('surface'), fg=self.theme.get_color('text_primary'))
        self.root.config(menu=menubar)
        
        # File menu
        file_menu = tk.Menu(menubar, tearoff=0, bg=self.theme.get_color('surface'), fg=self.theme.get_color('text_primary'))
        menubar.add_cascade(label="📁 File | 文件", menu=file_menu)
        file_menu.add_command(label="⚙️ Settings | 设置", command=self._open_settings, accelerator="Cmd+,")
        file_menu.add_separator()
        file_menu.add_command(label="📊 Open Reports | 打开报告", command=self._open_reports_folder, accelerator="Cmd+R")
        file_menu.add_command(label="📁 Open Data Folder | 打开数据文件夹", command=self._open_data_folder)
        file_menu.add_separator()
        file_menu.add_command(label="❌ Quit | 退出", command=self._quit_application, accelerator="Cmd+Q")
        
        # Edit menu
        edit_menu = tk.Menu(menubar, tearoff=0, bg=self.theme.get_color('surface'), fg=self.theme.get_color('text_primary'))
        menubar.add_cascade(label="✏️ Edit | 编辑", menu=edit_menu)
        edit_menu.add_command(label="🔍 Find Assignments | 查找作业", command=self._focus_search, accelerator="Cmd+F")
        edit_menu.add_command(label="🔄 Refresh | 刷新", command=self._refresh_assignments, accelerator="Cmd+R")
        ...
        # Tools menu
        tools_menu = tk.Menu(menubar, tearoff=0, bg=self.theme.get_color('surface'), fg=self.theme.get_color('text_primary'))
        menubar.add_cascade(label="🔧 Tools | 工具", menu=tools_menu)
        tools_menu.add_command(label="🔍 Check Assignments | 检查作业", command=self._check_assignments, accelerator="F5")
        tools_menu.add_command(label="🧪 Test Connection | 测试连接", command=self._test_connection)
        tools_menu.add_separator()
        tools_menu.add_command(label="🤖 AI Analysis | AI分析", command=self._run_ai_analysis, accelerator="Cmd+A")
        tools_menu.add_command(label="📊 Generate Report | 生成报告", command=self._generate_report, accelerator="Cmd+G")
        tools_menu.add_command(label="📧 Send Notifications | 发送通知", command=self._send_notifications)
        tools_menu.add_separator()
        tools_menu.add_command(label="📈 Statistics | 统计", command=self._show_statistics, accelerator="Cmd+S")
        
        # View menu
        view_menu = tk.Menu(menubar, tearoff=0, bg=self.theme.get_color('surface'), fg=self.theme.get_color('text_primary'))
        menubar.add_cascade(label="👁️ View | 查看", menu=view_menu)
        view_menu.add_command(label="🌞 Light Theme | 浅色主题", command=lambda: self._change_theme('professional_light'))
        view_menu.add_command(label="🌙 Dark Theme | 深色主题", command=lambda: self._change_theme('professional_dark'))
        ...
        view_menu.add_separator()
        view_menu.add_command(label="🔍 Toggle Search | 切换搜索", command=self._toggle_search, accelerator="Cmd+F")
        view_menu.add_command(label="📊 Toggle Sidebar | 切换侧边栏", command=self._toggle_sidebar)
        
        # Help menu
        help_menu = tk.Menu(menubar, tearoff=0, bg=self.theme.get_color('surface'), fg=self.theme.get_color('text_primary'))
        ...
        help_menu.add_command(label="🎥 Video Tutorial | 视频教程", command=self._open_video_tutorial)
    ...
    def _create_professional_stat_card(self, parent, emoji: str, label: str, value: str, var_name: str, row: int, col: int):
    ...
    def _create_content_header(self, parent):
        ...
        search_container = tk.Frame(search_frame, bg=self.theme.get_color('surface'), relief='flat', bd=1)
    ...
    def _setup_system_integration(self):
        ...
        try:
            self.tray_manager = ImprovedSystemTrayManager(
    ...
    def _generate_sample_assignments(self) -> List[Dict]:
        ...
        courses = ["Mathematics", "Physics", "Chemistry", "English Literature", "History", "Computer Science"]
    ...
    def _on_assignments_loaded(self):
        ...
        self._update_status(f"✅ Loaded {len(self.assignments)} assignments | 已加载{len(self.assignments)}个作业", "✅")
        ...
        for i, assignment in enumerate(assignments):
            ...
            if i < len(assignments) - 1:
                separator = tk.Frame(
                    ...
                    bg=self.theme.get_color('border'),
                    height=1
                )
    ...
    def _on_search_change(self, *args):
        ...
        for assignment in self.assignments:
            if (search_term in assignment.get('title', '').lower() or
                search_term in assignment.get('course', '').lower() or
                search_term in assignment.get('assignment_type', '').lower() or
                search_term in assignment.get('description', '').lower()):
    ...
    def _open_settings(self):
        ...
        # This would open the enhanced configuration dialog
        messagebox.showinfo(
            "Settings | 设置",
            "⚙️ Professional settings dialog would open here.\n⚙️ 专业设置对话框将在此处打开。\n\n"
            "This would include:\n这将包括：\n"
            "• ManageBac credentials | ManageBac凭据\n"
            "• AI Assistant configuration | AI助手配置\n"
            "• Theme and appearance | 主题和外观\n"
    ...
    def _generate_report(self):
        ...
        self._update_status("📊 Generating professional report... | 正在生成专业报告...", "📊", True)
...
"""
    ...
    def _refresh_assignments(self): self._check_assignments()
    def _focus_search(self): self.search_entry.focus_set()
    def _open_reports_folder(self): webbrowser.open(Path("reports").absolute().as_uri())
    def _open_data_folder(self): webbrowser.open(Path(".").absolute().as_uri())
    def _test_connection(self): messagebox.showinfo("Test", "🧪 Connection test | 连接测试")
    def _run_ai_analysis(self): messagebox.showinfo("AI", "🤖 AI Analysis | AI分析")
    def _send_notifications(self): messagebox.showinfo("Notifications", "📧 Notifications sent | 通知已发送")
    def _show_statistics(self): messagebox.showinfo("Stats", "📈 Statistics | 统计信息")
    def _show_all_assignments(self): self._display_assignments()
    ...
    def _show_high_priority(self): 
        ...
        self._apply_filters()
    def _toggle_search(self): self.search_entry.focus_set()
    def _toggle_sidebar(self): messagebox.showinfo("Sidebar", "📊 Sidebar toggle | 侧边栏切换")
    def _mark_all_read(self): messagebox.showinfo("Mark Read", "✅ All marked as read | 全部标记已读")
    def _clear_cache(self): messagebox.showinfo("Cache", "🧹 Cache cleared | 缓存已清除")
    def _change_theme(self, theme_name: str): 
        self.theme = ProfessionalTheme(theme_name)
        messagebox.showinfo("Theme", f"🎨 Theme changed to {theme_name} | 主题已更改为{theme_name}")
    def _open_documentation(self): webbrowser.open("https://github.com/Hacker0458/managebac-assignment-checker#readme")
    def _open_video_tutorial(self): webbrowser.open("https://github.com/Hacker0458/managebac-assignment-checker")
    def _open_community(self): webbrowser.open("https://github.com/Hacker0458/managebac-assignment-checker/discussions")
    def _report_bug(self): webbrowser.open("https://github.com/Hacker0458/managebac-assignment-checker/issues/new")
    def _feature_request(self): webbrowser.open("https://github.com/Hacker0458/managebac-assignment-checker/issues/new")
    def _check_updates(self): messagebox.showinfo("Updates", "🔄 Checking for updates | 检查更新")
