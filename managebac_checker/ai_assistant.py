...
class AIAssistant:
    ...
    def __init__(self, api_key: Optional[str] = None, language: str = 'en', 
                 enable_ai: bool = True, model: str = "gpt-3.5-turbo"):
        """
    ...
    def analyze_assignments(self, assignments: List[Assignment], 
    ...
    def _prepare_assignment_summary(self, assignments: List[Assignment], 
    ...
    def _generate_insights(self, assignment_summary: str) -> Dict[str, str]:
        ...
        try:
            ...
            response = self.client.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an educational assistant helping students manage their assignments."},
    ...
    def _generate_recommendations(self, assignment_summary: str, 
                                 insights: Dict[str, str]) -> List[str]:
        """Generate study recommendations | 生成学习建议"""
        if not self.client:
            return []
        ...
        try:
            ...
            response = self.client.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an educational assistant providing study recommendations."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=300
    ...
    def _generate_time_plan(self, assignments: List[Assignment], 
                           analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        """Generate time management plan | 生成时间管理计划"""
        if not self.client:
            return {}
        ...
        try:
            ...
            for assignment in assignments[:10]:
                assignment_list.append({
                    ...
                    'priority': analysis_data.get('priorities', {}).get(assignment.title, 'medium')
                })
            ...
            response = self.client.ChatCompletion.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a time management expert helping students plan their work."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=500
    ...
    def _generate_summary(self, insights: Dict[str, str], 
