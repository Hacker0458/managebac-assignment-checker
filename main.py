...
class ManageBacChecker:
    def __init__(self):
        ...
        self.priority_keywords = os.getenv(
            "PRIORITY_KEYWORDS", "exam,test,project,essay"
        ).split(",")
        ...
        if not self.email or not self.password:
            print(
                "Error: MANAGEBAC_EMAIL and MANAGEBAC_PASSWORD must be set in .env file"
            )
            sys.exit(1)
        ...
        try:
            ...
            for i, selector in enumerate(assignment_selectors):
                try:
                    ...
                    if elements:
                        print(
                            f"✓ 选择器 {i+1}: '{selector}' 找到 {len(elements)} 个元素"
                        )
                            ...
                            try:
                                ...
                                if title == "未知作业" and len(element_text) > 10:
                                    title = element_text[:100] + (
                                        "..." if len(element_text) > 100 else ""
                                    )

                                # 提取截止日期
                                for due_selector in due_date_selectors:
                                    try:
                                        due_element = await element.query_selector(
                                            due_selector
                                        )
                                        if due_element:
                                ...
                                for status_selector in status_selectors:
                                    try:
                                        status_element = await element.query_selector(
                                            status_selector
                                        )
                                        if status_element:
                                            status_text = (
                                                await status_element.text_content()
                                            )
                                ...
                                if is_unsubmitted:
                                    assignment_info = {
                                        ...
                                        "element_index": j,
                                        "full_text_preview": element_text[:200]
                                        + ("..." if len(element_text) > 200 else ""),
                                        "found_at": datetime.now().isoformat(),
                                    }
                        ...
                        if assignments:
                            print(
                                f"通过选择器 '{selector}' 找到 {len(assignments)} 个作业"
                            )
                            break  # 找到作业后停止搜索
    ...
    async def _search_page_text_for_assignments(
        self, page: Page, assignments: List[Dict[str, Any]]
    ) -> None:
        """
        在页面文本中搜索作业相关信息。
        """
        try:
            ...
            if any(keyword in page_content.lower() for keyword in assignment_keywords):
                ...
                for element in text_elements[:100]:  # 限制处理前100个元素
                    try:
                        ...
                        has_assignment_keyword = any(
                            keyword in text_lower for keyword in assignment_keywords
                        )

                        if (
                            has_assignment_keyword
                            and len(text) > 20
                            and len(text) < 500
                        ):
                            # 尝试判断是否为未提交的作业
                            ...
                            is_unsubmitted = any(
                                ...
                                for indicator in unsubmitted_indicators
                            )

                            if is_unsubmitted or (
                                "due" in text_lower and "submit" in text_lower
                            ):
                                assignments.append(
                                    {
                                        "title": text[:100]
                ...
                print(
                    f"文本搜索找到 {len([a for a in assignments if a.get('selector_used') == 'text_search'])} 个潜在作业"
                )
    ...
    def _remove_duplicate_assignments(
        ...
        for assignment in assignments:
            title_key = (
                assignment["title"][:50].lower().strip()
            )  # 使用前50个字符作为去重键
...
"""
    ...
    def save_reports(self, reports: Dict[str, str]) -> Dict[str, str]:
        ...
        for format_type, content in reports.items():
            ...
            file_extension = {"json": "json", "html": "html", "markdown": "md"}.get(
                format_type, "txt"
            )
    ...
    async def send_email_notification(
        ...
        发送邮件通知。
        """
        if not self.enable_notifications or not all(
            [
                self.smtp_server,
                self.email_user,
                self.email_password,
        ...
        try:
            ...
            with smtplib.SMTP(self.smtp_server, self.smtp_port) as server:
                ...
                server.sendmail(
                    self.email_user, self.notification_email, msg.as_string()
                )
    ...
    def _display_console_results(
        ...
        for i, assignment in enumerate(assignments, 1):
            ...
            print(
                f"      📊 状态: {assignment['status']} | {priority_emoji} {priority.upper()}优先级"
            )
